...
DRIVER_PATH = r"C:\Users\User\Desktop\server\chromedriver.exe" # Set your chronium path
    ...
    for team in soup.select("div.srch_team li"):
        ...
        for flag2 in range(0, 2):
            ...
            for player in soup.select("div.seqlist li"):
                ...
                if soup.select('.birth')[0].get_text().split('.')[0] == '':
                    player_data['age'] = 0
                else:
...
def crawl_NBA_quarter_gamePlayerStat():
...
def crawl_KBL_quarter_gamePlayerStat():
    global DRIVER_PATH
    global chrome_options
    driver = webdriver.Chrome(DRIVER_PATH, options=chrome_options)
